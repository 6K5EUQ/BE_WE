...
int col_cmp(int c, const BtleRecord& a, const BtleRecord& b){
    switch(c){
        ...
        case 3:  return a.pdu_type-b.pdu_type;
        case 4:  return memcmp(a.mac,b.mac,6);
        case 5:  return a.addr_type-b.addr_type;
        case 6:  return strcmp(a.name,b.name);
        case 7:  return (int)a.crc_ok-(int)b.crc_ok;
...
void draw_content(FFTViewer& v, bool just_opened){
    ...
    if(win_focus && !io.WantTextInput && io.KeyCtrl && !io.KeyShift &&
       ImGui::IsKeyPressed(ImGuiKey_C,false) && !sel.empty()){
        ...
        for(const BtleRecord& m : log){ if(!sel.count(msg_key(m))) continue;
            ...
            char line[320];
            snprintf(line,sizeof(line),"%s\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\n",
                ts,m.station[0]?m.station:"LOCAL",m.adv_chan,btle_pdu_name(m.pdu_type),
                mc,m.addr_type?"R":"P",m.name,m.crc_ok?"OK":"FAIL",inf);
            out+=line;
        }
    ...
    float detail_h = has_focus ? (focus.is_connect ? 132.f : 96.f) : 0.f;
    ...
    if(ImGui::BeginTable("##btle_tbl", 9, tf, ImVec2(W, table_h))){
        ...
        while(clip.Step()) for(int r=clip.DisplayStart;r<clip.DisplayEnd;r++){
            ...
            ImGui::TableSetColumnIndex(4); { char mc[18]; mac_str(m.mac,mc); modview::cell(mc, ImVec4(0.5f,0.8f,1.f,1.f)); }
            ImGui::TableSetColumnIndex(5); modview::cell(m.addr_type?"R":"P", ImVec4(0.6f,0.6f,0.6f,1.f));
            ImGui::TableSetColumnIndex(6); if(m.name[0]) ImGui::TextUnformatted(m.name);
            ImGui::TableSetColumnIndex(7); modview::cell(m.crc_ok?"OK":"FAIL",
