...
void CentralServer::handshake(int fd){
    ...
    if(type == CentralPktType::HOST_OPEN){
        ...
        // HOST에게 CONN_OPEN 알림
        {
            ...
            if(!central_send_mux(room->fd, je->conn_id, CentralMuxType::CONN_OPEN, nullptr, 0))
                printf("[Central] CONN_OPEN send to HOST failed conn_id=%u\n", je->conn_id);
        }
...
void CentralServer::join_loop(std::shared_ptr<JoinEntry> je, std::shared_ptr<HostRoom> room){
    ...
    while(je->alive.load() && room->alive.load()){
        ...
        // HOST에게 MUX 헤더 + BEWE 패킷 전달
        {
            ...
            if(!central_send_mux(room->fd, je->conn_id, CentralMuxType::DATA,
                               buf.data(), 9 + bewe_len)){
                int e = errno;
                printf("[Central] join_loop send to HOST failed conn_id=%u errno=%d(%s)\n",
                       je->conn_id, e, strerror(e));
    ...
    if(room->alive.load() && room->fd >= 0){
        std::lock_guard<std::mutex> lk(room->host_send_mtx);
        if(!central_send_mux(room->fd, je->conn_id, CentralMuxType::CONN_CLOSE, nullptr, 0))
            printf("[Central] CONN_CLOSE send to HOST failed conn_id=%u\n", je->conn_id);
    }
...
void CentralServer::rebuild_and_broadcast_ch_sync(std::shared_ptr<HostRoom> room){
    ...
    if(room->alive.load() && room->fd >= 0){
        ...
        if(!central_send_mux(room->fd, 0xFFFF, CentralMuxType::DATA,
            ...
            printf("[Central] rebuild_ch_sync: send to HOST failed room='%s' errno=%d(%s)\n",
                   room->station_id.c_str(), errno, strerror(errno));
        }
    }
}
